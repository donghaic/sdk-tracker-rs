...
pub fn find_map<'a>(addr: &str, lan: &str) -> Result<HashMap<&'a String, &'static str>, &'static str> {
    let data = find(addr, lan)?;
    let mut infos: HashMap<&String, &'static str> = HashMap::new();

    for x in 0..data.len() {
        let key = &IPDB.meta.fields[x];
        infos.insert(key, data[x]);
    }
    return Ok(infos);
}
